...
<html>
    ...
    <a href="/customer-personal-page-servlet">Personal page</a><br>
        <tr>
            <th>Order number:</th>
            <th><c:out value = "${order.getId()}"/></th>
        </tr><br>
        <tr>
            <th>Passport:</th>
            <th><c:out value = "${order.getPassportDetails()}"/></th>
        </tr><br>
        <tr>
            <th>Period:</th>
            <th>
        ...
        <tr>
            <th>Automobile:</th>
            <th><c:out value = "${order.getAutomobile().getManufacturer()}"/>
            <c:out value = "${order.getAutomobile().getName()}"/></th>
        </tr><br>
        <tr>
            <th>Driver:</th>
            <th><c:out value = "${order.isHasDriver()}"/></th>
        </tr><br>
        <tr>
            <c:if test="${order.isDenied() == true}">
                Denied. Reason: <c:out value="${order.getRejectionReason()}"/>
            </c:if>
        </tr><br>
        <c:choose>
        ...
        <tr>
            ...
            <c:if test="${order.getBill().isPayed() == false}">
                <c:if test="${service['class'].simpleName eq 'UserCustomerService'}">
                    <form action="/payment-servlet" method="POST">
                        <input type="hidden" id="bill_id" name="bill_id" value="${order.getBill().getOrderId()}">
            ...
            <c:if test="${service['class'].simpleName eq 'UserManagerService'}">
                                ...
                                <form action="/order-servlet" method="GET">
                                    <input type="hidden" id="automobile_id" name="automobile_id" value="${order.getAutomobileId()}">
